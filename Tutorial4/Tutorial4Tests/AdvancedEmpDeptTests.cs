...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, emp => emp.DeptNo,dept => dept.DeptNo,(emp, dept) => new { emp, dept }).Join(grades,empDept => true,grade => true,(empDept, grade) => new { empDept.emp, empDept.dept, grade }).Where(joined => joined.emp.Sal >= joined.grade.Losal && joined.emp.Sal <= joined.grade.Hisal).Select(joined => new { joined.emp.EName,joined.dept.DName,joined.grade.Grade }).ToList();
